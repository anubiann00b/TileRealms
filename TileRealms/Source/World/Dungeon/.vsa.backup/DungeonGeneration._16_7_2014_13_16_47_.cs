...
//lets display on mainstate first, since we can't see it until we run on main
namespace TileRealms.Source.World.Dungeon
{
    class DungeonGeneration
    {
        ...
        {  
            ...
            {
                Dungeon.Add(new Tile[(rooms.ElementAt(i).Width / 64) + 1, rooms.ElementAt(i).Height + 1]);
                for (int x = 0; x < Dungeon.ElementAt(i). + 1; x++)
                {
                    for (int y = 0; y < .Height + 1; y++)
                    {
                        Dungeon.ElementAt(i)[x, y] = Tile.TILE_WATER;
                    } //shit i fucked this up
                } //gg i brb lunch sorry try to work on it sorrrrrrrryryryryy
            }
        }
